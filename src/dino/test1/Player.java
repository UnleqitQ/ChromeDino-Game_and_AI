	...
	public void test(Obstacle obstacle) {
		...
		if (obstacle.bird) {
			...
			if (obstacle.posX<=100+(duck?90:120)) {
				if (obstacle.posX+(obstacle.thick?game.application.w1:game.application.w0)>=100) {
					if (posY*100 <= (obstacle.tall?game.application.h1:game.application.h0)) {
						alive = false;
					}
				}
			}
